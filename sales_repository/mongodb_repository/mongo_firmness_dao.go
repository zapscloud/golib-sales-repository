...
// FirmnessMongoDBDao - State DAO Repository
type FirmnessMongoDBDao struct {
	client     utils.Map
	businessID string
}
...
func (p *FirmnessMongoDBDao) InitializeDao(client utils.Map, businessId string) {
...
func (p *FirmnessMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	var results []utils.Map
	var bFilter bool = false
	log.Println("Begin - Find All Collection Dao", sales_common.DbFirmnesss)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, sales_common.DbFirmnesss)
	if err != nil {
		return nil, err
	}
...
func (p *FirmnessMongoDBDao) Get(otp_id string) (utils.Map, error) {
	...
	stages := []bson.M{}
	filter := bson.D{
		{Key: sales_common.FLD_FIRMNESS_ID, Value: otp_id},
		{Key: sales_common.FLD_BUSINESS_ID, Value: p.businessID},
		{Key: db_common.FLD_IS_DELETED, Value: false}}
...
// Find - Find by code
func (p *FirmnessMongoDBDao) Find(filter string) (utils.Map, error) {
	// Find a single document
	var result utils.Map
...
func (p *FirmnessMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Save - End", indata[sales_common.FLD_FIRMNESS_ID])

	return indata, err
}

// Update - Update Collection
func (p *FirmnessMongoDBDao) Update(otp_id string, indata utils.Map) (utils.Map, error) {
...
func (p *FirmnessMongoDBDao) Delete(otp_id string) (int64, error) {
...
func (p *FirmnessMongoDBDao) DeleteAll() (int64, error) {
...
// func (p *FirmnessMongoDBDao) appendListLookups(stages []bson.M) []bson.M {
